...
GRAPH STATE (type = "graph") — follow this EXACTLY:
  state = {
    ...
    queue: [3, 4],
    stack: [],
    actionLog: ["Visit node 1", "Enqueue neighbors 2, 3", "Dequeue node 2", "Enqueue neighbors 4, 5"],
    operation: "bfs",
    action: "Visiting node 2, enqueuing its neighbors",
    iteration: "Iteration 2 of ~4 max",
    distances: {},
    activeEdge: [1, 2]
  }

  GRAPH STATE RULES:
  - nodes: ALL nodes in the graph, each with integer id and string label (KEEP STABLE across all steps)
  - edges: ALL edges as [{from, to, weight?}] (KEEP STABLE across all steps)
  - currentNode: the integer id of the node being processed this step (null if none)
  - visited: cumulative array of node IDs already processed/visited (grows each step)
  - queue: current BFS queue contents (array of node IDs) — update every step
  - stack: current DFS stack contents (array of node IDs) — update every step (empty for BFS)
  - actionLog: CUMULATIVE list of all actions taken SO FAR (grows each step)
  - operation: one of "bfs" | "dfs" | "dijkstra" | "cycle" | "toposort"
  - action: human-readable description of what's happening this step
  - iteration: iteration description string, e.g. "Iteration 2 of ~5 max"
  - distances: for Dijkstra, a map of {nodeId: currentShortestDistance} — use null for unvisited/unreachable nodes (NOT large numbers like 999999), empty {} for BFS/DFS
  - activeEdge: [fromId, toId] of the edge being traversed this step (null if none)
...
// ─── SORTING INSIGHT ────────────────────────────────────────────────────────
router.post("/dsa/sorting-insight", async (req, res): Promise<void> => {
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a DSA expert. Return a concise JSON comparison insight for these 4 sorting algorithms: Bubble Sort, Insertion Sort, Merge Sort, Quick Sort.

Return ONLY valid JSON in this exact shape:
{
  "bestForLarge": "1 sentence: which algorithm is best for large datasets and why",
  "bestForSmall": "1 sentence: which is best for small or tiny datasets and why",
  "bestForNearlySorted": "1 sentence: which is best for nearly-sorted data and why",
  "generalTip": "1-2 sentences: practical rule of thumb for choosing a sorting algorithm"
}

Rules:
- Be specific and mention algorithm names
- Use plain, clear English
- No markdown, no lists inside strings`,
        },
        { role: "user", content: "Generate sorting algorithm comparison insight." },
      ],
      response_format: { type: "json_object" },
    });

    const raw = completion.choices[0]?.message?.content;
    if (!raw) { res.status(500).json({ error: "No response from AI" }); return; }
    res.json(JSON.parse(raw));
  } catch (err) {
    req.log.error({ err }, "Error generating sorting insight");
    res.status(500).json({ error: "Failed to generate insight" });
  }
});

// ─── SUGGEST SIMILAR PROBLEMS ───
router.post("/dsa/suggest", async (req, res): Promise<void> => {
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert DSA teacher. Given a problem, identify its pattern and suggest 3-4 similar problems the student should solve next to reinforce that pattern.

Return ONLY valid JSON in this exact shape:
{
  "pattern": "string — the detected algorithm pattern",
  "suggestions": [
    { "title": "string — exact LeetCode-style problem title", "difficulty": "Easy | Medium | Hard" },
...
Rules:
- Suggest 3-4 problems (not the same as the input)
- Mix difficulties (Easy, Medium, Hard)
- Prefer classic LeetCode problems
- Return ONLY JSON, no markdown`,
    ...
    req.log.error({ err }, "Error generating suggestions");
